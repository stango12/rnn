...
class RNNTheano:
    ...
    def theano_setup(self):
        ...
        def forward_prop_step(x_t, s_t_prev, U, V, W):
            s_t = T.tanh(T.dot(U, x_t) + W.dot(s_t_prev))
            o_t = T.nnet.softmax(V.dot(s_t))
            return [o_t[0], s_t]

        [o, s], updates = theano.scan(forward_prop_step, sequences=x, outputs_info=[None, dict(initial=T.zeros(self.hidden_dim))], non_sequences=[U, V, W], truncate_gradient=self.bptt_truncate, strict=True)
...
def train_with_sgd(model, x_train, y_train, learning_rate=np.float32(0.01), nepoch=100, evaluate_loss_after=5):
    ...
    for epoch in range(nepoch):
        ...
        if (epoch % evaluate_loss_after == 0):
            ...
            model.save_param("rnn-theano-parameters45k.npz") 
